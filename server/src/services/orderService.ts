...
async function createFortisTerminalForSerial(
  ...
  // Prefer the merchant's linked Fortis account (Admin → Fortis Gateway), else the configured
  // default location — Fortis has no MID field, so the link is explicit (see D8).
  const merchant = await prisma.merchant.findUnique({ where: { id: order.merchantId } });
  const locationId = merchant?.fortisLocationId ?? (await adapter.resolveLocationId({ mid: order.merchantMid ?? undefined })) ?? undefined;
